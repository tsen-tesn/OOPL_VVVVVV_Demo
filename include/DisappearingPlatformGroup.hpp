...
public:
    DisappearingPlatformGroup(const std::vector<glm::vec2>& positions, 
    ...
    const std::vector<std::shared_ptr<DisappearingPlatform>>& GetPlatforms() const { return m_Platforms; }

private:
    std::vector<std::shared_ptr<DisappearingPlatform>> m_Platforms;
};
